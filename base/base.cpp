...
base::UnitVector::UnitVector(double ra=0.0, double dec=0.0)
{
    uv.set_size(3);
    uv(0) = cos(ra)*cos(dec);
    uv(1) = sin(ra)*cos(dec);
...
double base::UnitVector::UnitVector::x() {return uv(0);}
double base::UnitVector::UnitVector::y() {return uv(1);}
double base::UnitVector::UnitVector::z() {return uv(2);}

base::RotationVector::RotationVector() {
    rv.set_size(3);
    rv(0) = 1.0;
    rv(1) = 0.0;
    rv(2) = 0.0;
...
base::RotationMatrix::RotationMatrix() {
    rm.eye(3,3);
}
base::RotationMatrix::RotationMatrix(UnitVector& uv, double yaw=0.0)
{
    rm.set_size(3,3);
...
base::Quaternion::Quaternion(UnitVector& uv, double yaw)
{
...
base::Quaternion::Quaternion(RotationMatrix& rm)
{
    q = rm2q(rm.rm);
//    q = normalise(q);
//    assert(norm(q) - 1.0 < 1e-6);
