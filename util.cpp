...
vec util::m2q(mat &m) {
    ...
    q(0) = 0.5*sqrt(1 + m(0,0) - m(1,1) - m(2,2))*sgn(m(1,2)-m(2,1));
    q(1) = 0.5*sqrt(1 - m(0,0) + m(1,1) - m(2,2))*sgn(m(2,0)-m(0,2));
    q(2) = 0.5*sqrt(1 - m(0,0) - m(1,1) + m(2,2))*sgn(m(0,1)-m(1,0));
    q(3) = 0.5*sqrt(1 + m(0,0) + m(1,1) + m(2,2));
    return q;
}
